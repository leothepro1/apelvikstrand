/* apelvikstrand.maps.bootstrap.js
   - Lottie overlay (snabb första feedback)
   - Laddar Mapbox CSS/JS
   - Laddar maps.core.js (skapar window.sektion73Map)
   - Väntar på map load
   - Laddar maps.ui.js (pins/filter/modal)
   - Tar bort overlay när UI är laddad + (så långt det går) efter nästa frame

   KRAV:
   - Behåll samma DOM-ids (#sektion73MapRoot, #sektion73MapCanvas)
   - Ändra inte karta/UI-logic, bara load order
*/
(function () {
  "use strict";
  ...
  // -----------------------------
  // Overlay mål: tidig synlighet + snabb removal
  // -----------------------------
  ...
  function sektion73EnsureOverlayCss() {
    ...
    css.type = "text/css";
    css.textContent =
      "#" + sektion73OverlayId + "{position:fixed;inset:0;z-index:2147483647;display:grid;place-items:center;background:#fff;opacity:1;transition:opacity .28s ease;pointer-events:auto}" +
      "#" + sektion73OverlayId + ".sektion73LottieHiding{opacity:0;pointer-events:none}" +
      "#" + sektion73OverlayId + " .sektion73LottieInner{width:min(320px,74vw);height:min(320px,74vw);display:grid;place-items:center}" +
      "#" + sektion73OverlayId + " .sektion73LottiePoster{width:100%;height:100%;display:grid;place-items:center}" +
      "#" + sektion73OverlayId + " .sektion73LottieSpinner{width:60px;height:60px;border-radius:999px;border:4px solid rgba(14,19,24,.12);border-top-color:rgba(14,19,24,.55);animation:sektion73Spin .85s linear infinite}" +
      "#" + sektion73OverlayId + " dotlottie-wc{width:100%;height:100%;display:block}" +
  ...
  function sektion73LoadScript(src, onDone, opts) {
    ...
    s.onerror = function () {
      // Fail-open: lämna inte overlay kvar om vi failar
  ...
  function sektion73MountLottiePlayer() {
    // Lottie ska inte blocka map-load; kör efter första paint
    sektion73LoadScript(
  ...
  function sektion73WaitForCoreMapThen(cb) {
    var tries = 0;
    var maxTries = 60 * 12;
  ...
  function sektion73BootInteractiveMap() {
    ...
    sektion73LoadCss(sektion73MapboxCssHref, function () {
      ...
      sektion73LoadScript(sektion73MapboxJsSrc, function () {
        // 3) maps.core.js (skapar window.sektion73Map)
        sektion73LoadScript(sektion73MapsCoreSrc, function () {
          // 4) vänta in map-instans och map load
          sektion73WaitForCoreMapThen(function (m) {
            if (!m) {
              // Fail-safe: släpp overlay om något gick snett
              sektion73HideOverlayWhenAllowed();
              return;
            }

            // Kör UI EFTER map style load => minimerar race och blir stabilt
            m.once("load", function () {
              sektion73LoadScript(sektion73MapsUiSrc, function () {
                // 5) UI laddad, släpp overlay
